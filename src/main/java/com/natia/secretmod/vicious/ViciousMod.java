...
public class ViciousMod {
    ...
    public ViciousMod(ViciousConfig vicious_config, String name) {
        ...
        this.name = name;
        configFile = new File(Minecraft.getMinecraft().mcDataDir.getAbsolutePath() + "\\config\\" + name + ".cfg");
    ...
    public void saveConfig() {
        ...
        for(int var5 = 0; var5 < var4; ++var5) {
            ...
            if (variable.isAnnotationPresent(AddConfig.class)) {
                try {
                    AddConfig config = (AddConfig)variable.getAnnotation(AddConfig.class);
                    List<Object> defined = new ArrayList();
                    defined.add(config.description());
                    defined.add(config.type());
            ...
            for (Field variable : config.getClass().getDeclaredFields()) {
                if (variable.isAnnotationPresent(AddConfig.class)) {
                    ...
                    if (data.containsKey(config.name()) && config.type() == ConfigType.HUD)
                        variable.set(config, new HudElement((List<Integer>)((List<Object>)data.get(config.name())).get(2)));
                    else if (data.containsKey(config.name()))
                        variable.set(config, ((List<Object>)data.get(config.name())).get(2));
                }
            }
